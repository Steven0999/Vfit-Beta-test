    ...
    // ==========================================================================
    const VFIT_APP_VERSION = '2.1.0-beta.8';
    const VFIT_STATE_SCHEMA_VERSION = 7;
    ...
    function mergeStateSnapshots(localInput, remoteInput) {
        ...
        const arrayFields = new Set([
            ...
            'createdMeals', 'customFoods', 'barcodeFoods', 'habits', 'userGoals', 'cardioLogs',
        ...
        const barcodeKey = (item, index) => openFoodFactsBarcodeKey(item && (item.scannedBarcode || item.barcode)) || `barcode:${index}`;
        ...
        content.innerHTML = `
            <article class="border-2 border-slate-200 rounded-[2rem] p-4 sm:p-6">
                ...
                <div class="mt-6 bg-amber-50 border border-amber-200 p-4 rounded-xl">
                    ...
                    <p class="text-[11px] text-amber-800 mt-1">${requirementLabels.length ? `Your saved requirements: ${escapeHtml(requirementLabels.join(' · '))}. ` : ''}${profile.notes ? `Your note: ${escapeHtml(profile.notes)}. ` : ''}Possible recipe flags: ${escapeHtml(allergenLabels.length ? allergenLabels.join(', ') : 'none inferred from the title')}. Always check every label, substitute only with products safe for you, prevent cross-contamination and verify halal/kosher certification where required.</p>
                </div>
            </article>`;
...
function shiftFoodIdeasHTML(emphasiseNight, dateKey) {
    ...
    function saveState(options) {
        ...
        try {
            ...
            if (error && (error.name === 'QuotaExceededError' || error.code === 22)) {
                showToast('Storage is full. Export a backup before removing older photos.', 6500);
            } else {
    ...
    function mountBarcodeScannerSurface() {
        ...
        if (barcodeScannerEmbedded && mealSlot) {
            modal.style.display = 'none';
            mealSlot.classList.remove('hidden');
            if (card.parentElement !== mealSlot) mealSlot.appendChild(card);
            requestAnimationFrame(() => {
                try { mealSlot.scrollIntoView({ behavior: 'smooth', block: 'nearest' }); } catch (error) {}
            });
            return true;
        }

        if (card.parentElement !== modal) modal.appendChild(card);
        if (mealSlot) mealSlot.classList.add('hidden');
        modal.style.display = 'flex';
        return true;
    }
    ...
    async function openBarcodeScanner() {
        ...
        if (title) title.textContent = barcodeScanMode === 'meal' ? 'Scan Ingredient Barcode' : 'Scan Barcode';
        if (input) input.value = '';
    ...
    /** Resize and progressively compress a photo to a localStorage-friendly size. */
